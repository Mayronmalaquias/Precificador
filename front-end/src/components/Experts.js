...
export default function ExpertsPages({
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (alive) setState({ loading: false, error: "Erro ao carregar dados.", data: null });
      }
    }
    load();
    return () => { alive = false; };
  ...
  if (state.loading) return <div className="container my-4 text-center">Carregando...</div>;

  const dataset = state.data || { citiesOrder: [], experts: [] };

  return mode === "expert" ? (
    <ExpertView slug={slug} dataset={dataset} />
  ) : (
...
/* --- HUB VIEW (Ajustada para o seu novo CSS com Grid) --- */
function HubView({ dataset, expertPageBasePath }) {
  ...
  const { finalCities, byCity } = useMemo(() => {
    ...
    const extraCities = Array.from(map.keys()).filter(c => !citiesOrder.includes(c)).sort();
      ...
      <div className="expert-container my-4">
        <div className="row mb-3">
          <div className="col-12">
            ...
            <p className="expert-subtitle">Escolha uma região e conheça nossos especialistas.</p>
          </div>
        </div>
              ...
              <div className="city-cards">
                {byCity.get(city).length > 0 ? (
                  byCity.get(city).map((ex) => (
                    <a key={ex.slug} className="expert-card" href={`${expertPageBasePath}?slug=${ex.slug}`}>
                      <div className="expert-card-photo">
                        <img src={ex.photo} alt={ex.name} />
                      </div>
                      <div className="expert-card-body">
                        <p className="expert-card-name">{ex.name}</p>
                        {ex.roleTag && <p className="expert-card-tag">Expert {city}</p>}
                      </div>
                    </a>
                  ))
                ) : (
...
function ExpertView({ slug, dataset }) {
  const expert = useMemo(() => 
    ensureArray(dataset.experts).find(e => String(e.slug).toLowerCase() === String(slug).toLowerCase())
  , [dataset.experts, slug]);

  if (!expert) return <div className="container my-4">Expert não encontrado.</div>;

  const waLink = `https://api.whatsapp.com/send?phone=${normalizePhoneBR(expert.contacts?.whatsapp)}&text=${encodeURIComponent(`Olá, ${expert.name}! Vim pela página de Experts.`)}`;
    ...
    <main className="container-fluid expert-page">
      <div className="container my-4">
          ...
          <div className="expert-profile-grid">
            ...
            <div>
              ...
              <div className="expert-cities">
                {ensureArray(expert.cities).join(" • ")} {expert.roleTag && ` • ${expert.roleTag}`}
              </div>
              {expert.bioShort && <p className="expert-bio-short">{expert.bioShort}</p>}
              <div className="expert-bio-full-wrap">
                {String(expert.bioFull || "").split(/\n\s*\n/g).map((p, i) => (
                  <p key={i} className="expert-bio-full">{p}</p>
                ))}
              </div>
              <div className="expert-cta">
                <div className="expert-cta-box">
                  <a className="btn btn-soft-primary" target="_blank" rel="noopener" href={waLink}>Falar no WhatsApp</a>
                  <a className="btn btn-soft" href="#expert-properties">Ver imóveis</a>
                  {expert.contacts?.instagram && (
                    <a className="btn btn-soft" target="_blank" rel="noopener" href={expert.contacts.instagram}>Instagram</a>
                  )}
                </div>
              </div>
            </div>
        ...
        <div className="row mt-5" id="expert-properties">
          <div className="col-12"><h2 className="expert-section-title">Imóveis deste expert</h2></div>
          {ensureArray(expert.properties).map((p, i) => (
            <div className="col-12 col-md-6 col-lg-4 mt-3" key={i}>
              <a className="property-card" href={p.url} target="_blank" rel="noopener">
                <div className="property-thumb"><img src={p.thumb} alt={p.title} /></div>
