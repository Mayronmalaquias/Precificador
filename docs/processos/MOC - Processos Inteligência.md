...
| 1.10 | **Catálogo completo do Imoview** — `exibircaptadores`/`exibiranexos`, fotos e anexos, publicação por portal, e a regra: parâmetro errado não dá erro, dá resposta errada | ✅ 🆕 | [[1.10 - Catálogo Completo do Imoview]] |
...
| [[_Registro - 2026-09-02]] | **Rodada 29/08–02/09.** Tema: consultas que descartavam em silencio o que deviam mostrar. **138 visitas com pendencia real invisiveis** no Painel de Tarefas (join interno na tabela de flags); lead abrindo **o registro errado** (elo `id_legado` por cliente+telefone — 8.746 de 31.829 apontam para outro lead, e o `.first()` contornava a checagem de equipe); dropdowns que apagavam as proprias opcoes (Leads e Clientes). **Visao do Diretor de 48s para 16s** (N+1 de 174 consultas), com zero divergencia em 57 recortes — e a descoberta de que ~2,9s de toda requisicao e handshake do RDS. Publicacao por portal (**DF imoveis com zero super destaque**), fotos e anexos do Imoview, ergonomia com barra fixa/gaveta/chips em 4 telas, e a `main` finalmente alinhada com a `dev_miron`. ⚠️ Carimbo `sistema_corte_20260826` segue silenciando 42.260 leads |
